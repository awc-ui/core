...
export const VERTICALS = Object.freeze([
  ...
  {
    id: 'community',
    title: 'Corvus — Friends & Groups',
    /*
     * REACT ONLY WHILE THE OTHER FOUR PORTS ARE WRITTEN.
     *
     ...
     * of them red on a build that is simply not written. Each entry lands with
     * its port, exactly as Lyra's did — and `localeRouted` must never name a
     * framework `builds` lacks, which is a mistake this file has already
     * recorded once.
     */
    builds: Object.freeze([
      { framework: 'react', output: 'dist' },
    ...
    reference: 'react',
    /* `html` goes in here only now that the port exists — see the note on the
