...
`medium` grows 112px → **136px** and `large` 120px → **152px** the moment a
...
<ComponentDemo
  ...
  label="Medium — the subtitle adds 24px"
...
<ComponentDemo
  ...
  label="Large — the subtitle adds 32px"
...
<ComponentDemo
  ...
  label="Both frames are under 600px — the tune icon is hidden in both, only the hint inset differs"
  storyId="navigation-app-bar--responsiveness"
/>

:::note[The wide tier isn't visible on this page]
The docs content column caps these bars at roughly 510px, so both frames above
...
fixed 800px, to see the ≥ 600px tier with every in-pill icon showing.
:::
## Scrolled state
...
The row floors at 48px and the icon touch targets at 32px — both under the 48px
...
`density` floors the row at 48px and the touch target at 32px.
