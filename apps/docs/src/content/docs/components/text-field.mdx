...
`density="-1…-4"` compresses the container from 56dp down to 40dp, 4dp per rung,
...
The rungs shrink the field box with the row — 56dp at `0` down to 40dp at `-4`
— and the internal clear / password-toggle / speech buttons shrink with it
(24dp glyphs taper 1px per rung to an 18px floor). 40dp is under the 48px that
...
inherits `-2` and renders at 48dp like its neighbours. To restore the default
