...
| A chip a layout has stretched, label flush left | `content-align="start"` |
...
<md-chip appearance="filled" color="info">Updated</md-chip>
...
`content-align` decides where the content row sits when the chip is **wider than
its content**. It does nothing to a normal chip: `:host` is `display: inline-flex`,
so a chip shrink-wraps its label and there is no spare width to align within. It
starts to matter the moment a layout gives the chip a width — a grid column,
`align-self: stretch`, an explicit `inline-size` — and then the default `center`
leaves the label adrift in the middle of the box, which reads as an empty input
rather than a chip.

Reach for `start` when a stretched chip is labelling something beneath it: the
two then share a leading edge and read as one unit.

<ComponentDemo
  html={`<div style="display:grid;grid-template-columns:repeat(3,220px);gap:12px;">
  ...
  label="Three alignments, each in a 220px grid column"
  storyId="selection-chip--content-alignment"
/>
...
[RTL](/behaviour/rtl/). `content-align` is logical too — `start` is the
