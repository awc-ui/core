...
| [`md-slider`](/components/slider/) | Price range: `range` + `value-start`/`value-end`, `step="5"`, per-thumb names via `label-start`/`label-end`. The text readout below carries the values — `value-indicator` bubbles are always-on and would overflow the sheet at the endpoints. `mdInput` recounts continuously during the drag. |
