...
const dotWrap = { position: 'relative', display: 'inline-flex', inlineSize: 12, blockSize: 12,
  '--md-status-dot-inset-end': '0px', '--md-status-dot-inset-block-end': '0px' };
    ...
    <div style="flex: 1; display: flex; flex-direction: column;">
      ...
      <MdList v-if="folder === 'inbox'" selection-mode="single-select" label="Inbox conversations"
        ...
        <MdListItem v-for="m in visible" :key="m.id" :leading-avatar-name="m.sender"
          ...
          <span slot="trailing" style="display: flex; align-items: center; gap: 8px;">
            {{ m.clock }}
            <span v-if="m.unread" style="position: relative; display: inline-flex; inline-size: 12px; block-size: 12px; --md-status-dot-inset-end: 0px; --md-status-dot-inset-block-end: 0px;">
              <MdStatusDot label="Unread" style="--md-status-dot-color: var(--md-sys-color-primary);" />
            </span>
          </span>
        </MdListItem>
      </MdList>
