...
import { buildEncore } from './build.mjs';
...
export async function buildFrameworks(destination = resolve(appRoot, 'dist')) {
  ...
  try {
    await buildEncore(resolve(staging, 'html'));
    ...
    await run([require.resolve('@angular/cli/bin/ng.js'), 'build', 'encore-angular', '--output-path', resolve(staging, 'angular-output')], resolve(appRoot, 'frameworks/angular'));
    ...
    if (resolve(destination) === resolve(appRoot, 'dist')) {
      await writeFile(resolve(destination, 'index.html'), '<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Encore</title><script src="./redirect.js"></script></head><body><a href="./html/">Open Encore</a></body></html>');
    ...
    console.log('Encore: HTML, React, Vue, Angular, and Svelte builds ready.');
  } finally {
    await rm(assets, { recursive: true, force: true });
    await rm(staging, { recursive: true, force: true });
  }
