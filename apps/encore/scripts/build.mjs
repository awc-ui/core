...
export async function buildEncore(destination = resolve(appRoot, 'dist')) {
  ...
  await writeFile(index, (await readFile(index, 'utf8')).replace('<!-- encore:loading -->', startupMarkup()));
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  await buildEncore();
  console.log('Encore built successfully → dist/');
}
