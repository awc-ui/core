...
export async function generateFrameworkShells() {
  ...
  const index = (await readFile(resolve(appRoot, 'src/index.html'), 'utf8'))
    .replace('<!-- frame:loading -->', startupMarkup())
  ...
  for (const name of ['react', 'vue', 'svelte', 'angular']) {
    ...
    const host = `<div class="frame-framework-host">${html}</div>`;
    if (name === 'react') {
      await write(name, 'FrameShell.jsx', `// ${preface}\nimport { useLayoutEffect, useRef } from 'react';\nimport { mountFrame } from '../../src/app.js';\nexport function FrameShell() {\n  const root = useRef(null);\n  useLayoutEffect(() => mountFrame(root.current, { framework: 'react' }), []);\n  return <div ref={root} className="frame-framework-host">${jsx(parseFragment(html))}</div>;\n}\n`);
    } else if (name === 'vue') {
      await write(name, 'FrameShell.vue', `<!-- ${preface} -->\n<script setup>\nimport { ref, onMounted, onBeforeUnmount } from 'vue';\nimport { mountFrame } from '../../src/app.js';\nconst root = ref(null);\nlet dispose;\nonMounted(() => { dispose = mountFrame(root.value, { framework: 'vue' }); });\nonBeforeUnmount(() => dispose?.());\n</script>\n<template>${host.replace('class="frame-framework-host"', 'ref="root" class="frame-framework-host"')}</template>\n`);
    } else if (name === 'svelte') {
      await write(name, 'FrameShell.svelte', `<!-- ${preface} -->\n<script>\nimport { onMount } from 'svelte';\nimport { mountFrame } from '../../src/app.js';\nlet root;\nonMount(() => mountFrame(root, { framework: 'svelte' }));\n</script>\n${host.replace('class="frame-framework-host"', 'bind:this={root} class="frame-framework-host"')}\n`);
    } else {
      await write(name, 'src/shell.html', `<!-- ${preface} -->\n${html}\n`);
    }
    await write(name, name === 'angular' ? 'src/index.html' : 'index.html', name === 'angular'
      ? index.replace('<div id="app"></div>', '<div id="app"><frame-shell></frame-shell></div>')
