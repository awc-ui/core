<script lang="ts">
  ...
  function toggleTheme() {
    ...
    document.documentElement.style.backgroundColor = dark ? 'var(--md-sys-color-background)' : '';
  }
...
<div class="app">
  <div class="content">
    <h1>AWC UI Svelte Integration</h1>
    <p class="subtitle">All 34 Material Design 3 components</p>

    <section>
      <md-button variant="tonal" role="button" tabindex="0" on:click={toggleTheme} on:keydown={(e) => e.key === "Enter" && toggleTheme()}>
    ...
    <section>
      ...
      <div class="row">
        <md-checkbox
          checked={checked}
          on:mdChange={(e) => (checked = e.detail.checked)}
        ></md-checkbox>
        <md-radio name="r" value="a"></md-radio>
        <md-radio name="r" value="b"></md-radio>
        <md-switch
          selected={switched}
          on:mdChange={(e) => (switched = e.detail)}
        ></md-switch>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="row">
        <md-button variant="outlined" on:mdClick={() => (dialogOpen = !dialogOpen)}>
          {dialogOpen ? 'Close Dialog' : 'Open Dialog'}
        </md-button>
        <md-dialog open={dialogOpen} headline="Demo Dialog">
          <p>This is a static dialog preview.</p>
          <div slot="actions">
            <md-button variant="text" on:mdClick={() => (dialogOpen = false)}>Cancel</md-button>
            <md-button variant="filled" on:mdClick={() => (dialogOpen = false)}>Confirm</md-button>
          </div>
        </md-dialog>
        <md-button variant="outlined" on:mdClick={() => (snackbarOpen = !snackbarOpen)}>
          {snackbarOpen ? 'Hide Snackbar' : 'Show Snackbar'}
        </md-button>
        <md-snackbar open={snackbarOpen} message="File saved successfully" action="Undo"></md-snackbar>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="row" style="align-items:flex-start">
        {#each ['elevated', 'filled', 'outlined'] as v}
          <md-card variant={v} style="width:180px">
            <div slot="header" style="padding:16px 16px 0;font-weight:500;text-transform:capitalize">{v}</div>
            <p style="padding:16px;margin:0;font-size:14px;color:var(--md-sys-color-on-surface-variant)">Svelte card</p>
          </md-card>
        {/each}
      </div>
    </section>
    ...
    <section>
      ...
      <div class="row" style="align-items:flex-start">
        ...
        <div>
          <md-button id="menu-anchor" variant="outlined" on:mdClick={() => (menuOpen = !menuOpen)}>
    ...
    <section>
      ...
      <div style="display:grid;gap:16px">
        <div>
          <md-button variant="outlined" on:mdClick={() => (bottomSheetOpen = !bottomSheetOpen)}>
        ...
        <md-bottom-sheet open={bottomSheetOpen} variant="standard" headline="Quick actions" on:mdClose={() => (bottomSheetOpen = false)}>
          <md-list>
...
<style>
  ...
  :global(body) { margin: 0; background-color: var(--md-sys-color-background, #FFFBFE); }

  .app { background-color: var(--md-sys-color-background); color: var(--md-sys-color-on-background); min-height: 100vh; }
  .content { padding: 24px; font-family: Roboto, sans-serif; max-width: 900px; }
