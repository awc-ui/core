#!/usr/bin/env node
/**
 * Does the Vue build actually work in a browser?
 *
 ...
 * one of its, in the same order and under the same headings, because the four
 * ports exist to prove the same application can be written five ways — and a
 * port that quietly does less is not a port. It asserts on OUTCOMES a reader
 * could see, never on how Vue happened to implement them.
 *
 * Starts its own server, so it needs nothing running:
 *   pnpm --filter @awc-ui/showcase-music-vue build
 *   pnpm --filter @awc-ui/showcase-music-vue verify
 */
...
const PORT = 4381;
const BASE = `http://localhost:${PORT}${SHOWCASE_BASE}/vue`;
