#!/usr/bin/env node
/**
 * Does the Svelte build actually work in a browser?
 *
 ...
 * one of its, in the same order and under the same headings, and asserts on
 * OUTCOMES a reader could see rather than on how Svelte implemented them.
 *
 * Starts its own server, so it needs nothing running:
 *   pnpm --filter @awc-ui/showcase-music-svelte build
 *   pnpm --filter @awc-ui/showcase-music-svelte verify
 */
...
const PORT = 4382;
const BASE = `http://localhost:${PORT}${SHOWCASE_BASE}/svelte`;
