...
function appBar(t) {
  ...
  return html`<md-app-bar${attrs({
    ...
    'leading-icon': 'menu',
    'leading-icon-label': t('music.nav.menu'),
  })}>
    <span slot="headline" class="shell__brand">${t('music.app.brand')}<md-tooltip${attrs({
      text: t('music.app.demoNotice'),
    })}><md-chip${attrs({
    ...
    <md-avatar slot="trailing"${attrs({
      ...
      label: t('music.app.viewer', { name: profile.displayName }),
      size: 'small',
    })}></md-avatar>
  </md-app-bar>`;
}
  ...
  return html`<md-navigation-rail${attrs({
    ...
    ${DESTINATIONS.map(
      (destination) => html`<md-navigation-rail-tab${attrs({
        ...
        /* NO BADGE ANYWHERE IN THIS APP: a badge is for a count somebody else
           is blocked on, and a queue length is the reader's own state. */
...
export function screen(t, { locale, here, title, subtitle, crumbs = [], aside, actions, children }) {
  return html`<div class="shell">
        ...
        ${actions
          ? html`<div class="screen-toolbar">
              <md-toolbar${attrs({
                variant: 'floating',
                color: 'vibrant',
                'aria-label': t('music.nav.toolbar'),
              })}>${actions}</md-toolbar>
            </div>`
          : null}
    ...
    <!--
      ABOVE THE ROUTER AND BELOW THE SCREEN, in the four SPA builds' terms —
      here it is simply written into every page. client/transport.mjs restores
      what the reader was doing out of sessionStorage, which is the only way a
      static site can keep anything across a page load.
    -->
    ${transport(t, { queue: getQueue() })}
  ...
  <!-- LAST IN THE DOCUMENT, matching the other four, where the snackbar is
       rendered after everything else. It is position:fixed, so where it sits
       makes no visual difference — but the parity census compares ORDER. -->
  ${snackbar(t)}`;
}
...
export function emptyState(message, { hint } = {}) {
  /* THE GLYPH AND THE CLASSES MATCH THE OTHER FOUR BUILDS EXACTLY — the parity
     census counts the ligature's text, and a bare paragraph here is nine
     characters short on every screen with an empty panel on it. */
  return html`<div class="empty">
    <span class="material-symbols-outlined empty__icon" aria-hidden="true">music_off</span>
    <p class="empty__message">${message}</p>
    ${hint ? html`<p class="empty__hint">${hint}</p>` : null}
  </div>`;
}
...
export function snackbar(t) {
  /*
   * NO `closeable`, MATCHING THE OTHER FOUR BUILDS. Corvus's snackbar carried
   * one, and it renders a dismiss button inside the component's shadow root —
   * one extra `md-icon-button` and one extra `md-ripple` on every page, which
   * the parity census reported on all nine screens at once. The bar hides
   * itself after four seconds here as it does everywhere else, so the button
   * was an affordance one build had and four did not.
   */
