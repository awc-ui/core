 ...
 *
 * THE REACT BUILD'S SUITE IS THE SPECIFICATION. Every assertion here answers
 * one of its, in the same order and under the same headings.
 *
 ...
 *
 *   - The transport cannot be held in memory across a navigation, so it is
 *     written to `sessionStorage` and read back. The assertion is the same one
 *     a reader would make — the track is still loaded, still playing, and the
 *     playhead has kept going — but the mechanism under it is persistence
 *     rather than a provider, and section 2 says so.
 *   - React reaches its not-found screen through `pushState`, because a static
 *     host answers 404 for an unknown slug before the app runs. This build IS
 *     that static host, so the 404 is asserted directly.
 *   - Locale is in the URL rather than in a query parameter, so Arabic is a
 *     different document and is fetched as one.
 *
 * Starts its own server, so it needs nothing running:
 *   pnpm --filter @awc-ui/showcase-music-html build
 *   pnpm --filter @awc-ui/showcase-music-html verify
 */
...
const PORT = 4384;
const BASE = `http://localhost:${PORT}${SHOWCASE_BASE}/html`;
...
/**
 * Press something — and survive the page load if it was a link.
 *
 * EVERY NAVIGATION IN THIS BUILD IS A REAL PAGE LOAD. Pressing a link tears
 * down the execution context the `evaluate` is running in, and Puppeteer
 * reports that as "Execution context was destroyed" — which is not a failure
 * here, it is the architecture. The four SPA builds never hit it because their
 * routers keep the document.
 *
 * So a destroyed context is caught and waited out rather than thrown. Anything
 * that is NOT a navigation still surfaces normally.
 */
async function click(page, selector, index = 0) {
  ...
  await page
    .waitForFunction(() => document.querySelectorAll('.hydrated').length > 4, { timeout: 20000 })
    .catch(() => {});
  await settle(page);
}
...
/* ----------------------------------------------------- 10. the not-found */
{
   ...
   *
   * React drives its not-found screen through `pushState`, because a static
   * host answers 404 for an unknown slug before its app has run. This build IS
   ...
   * status code. The screen still exists (`screens/not-found.mjs`) and the four
   * drills render it when an id in their own argument does not resolve; it is
   * simply not reachable by navigation, because nothing navigates here.
   */
  const response = await browser.newPage().then(async (page) => {
    const res = await page.goto(`${BASE}/album/no-such-record/`, { waitUntil: 'domcontentloaded' });
  ...
  ok('rather than silently showing a track list', response.rows === 0);
}
...
/* --------------------------------------------------------- 11. Arabic */
{
  ...
  const shape = await page.evaluate(() => {
    ...
    return {
      ...
      /* Every in-app link has to keep the reader in Arabic. One that forgot the
         prefix is how a locale-routed site drops people back into English three
         clicks in — the failure `localeHref` exists to prevent, and one this
         build can check because the evidence is in the markup. */
      links: [...document.querySelectorAll('.shell__main a[href^="/showcase/"]')].length,
      arabicLinks: [...document.querySelectorAll('.shell__main a[href^="/showcase/"]')].filter((a) =>
        a.getAttribute('href').includes('/html/ar/'),
      ).length,
    };
  });
  ...
  /* mm:ss stays in Latin digits in every locale — see `clock()` in the kit.
     Arabic-Indic digits in the scrubber beside Latin bar numbers on the ruler
