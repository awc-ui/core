 ...
 * navigation bar (compact), the back row, screen heading, screen toolbar, and
 * the showcase dock.
 *
 ...
 *
 * NO BREADCRUMBS IN THIS VERTICAL, which is the one structural difference from
 * the three consoles next door. They drill a hierarchy and a trail is true
 * wherever you came from; a post here is reachable from the feed, an explore
 * grid, four profiles and a notification, so a trail would be an outright lie
 * for most readers. The row that holds a trail elsewhere holds a single BACK
 * affordance here, which claims only one step to somewhere that definitely
 * contains this thing. `backFor` in the kit carries the full argument.
 *
...
import { DESTINATIONS, getViewer, type CrumbSpec } from '@awc-ui/showcase-kit/music';
import { isPlainActivation, usePathname, useRouter } from '@/lib/router';
import { crumbsFor, destinationIndex, route, withBase } from '@/lib/routes';
...
function AppBar() {
  ...
  return (
    <md-app-bar
      ...
      class="shell__appbar"
      variant="small"
      subtitle={t('music.app.title')}
      leading-icon={compactNav ? undefined : 'menu'}
      leading-icon-label={compactNav ? undefined : t('music.nav.menu')}
    >
      {/*
        THE DISCLAIMER IS PART OF THE CHROME, not a footnote.

        Every person in this app is invented, every caption was written for
        it, and every picture is generated artwork — and it is all presented in
        the shape of a real social app, which is exactly the combination a
        reader could mistake for one. The stakes are higher here than in the
        three consoles: those invent a bank, this one invents PEOPLE, with
        faces and opinions. So the disclaimer sits in the app bar, beside the
        brand it qualifies, visible on every screen rather than at the bottom
        of one.
      ...
      */}
      <span slot="headline" className="shell__brand">
        {t('music.app.brand')}
        <md-tooltip text={t('music.app.demoNotice')}>
          <md-chip
            label={t('music.app.demo')}
            appearance="outlined"
            color="warning"
            icon="science"
          />
      ...
      {/* WHO IS SIGNED IN, and nothing else. The three consoles put a reporting
          ...
          is not "as of" anything a reader needs told, and the relative
          timestamps say when each post was without a reference date in the
          chrome. What DOES need saying on every screen is whose account this
          is, because "your posts" and "people you follow" are only meaningful
          once you know who "you" is. */}
      <div slot="trailing" className="shell__meta">
        <span>{viewer.displayName}</span>
        <span className="shell__handle">@{viewer.handle}</span>
      </div>
      {/* `src` is the viewer's generated portrait; `name` supplies the initials
          for the moment before it decodes. Presentational — the avatar is not a
          control and opens nothing; the Profile destination is how you get
          there. */}
      <md-avatar
        slot="trailing"
        src={viewer.art.src}
        name={viewer.displayName}
        label={t('music.app.viewer', { name: viewer.displayName })}
        size="small"
      />
    </md-app-bar>
  );
      ...
      */}
      ...
      {DESTINATIONS.map((destination) => (
        <md-navigation-rail-tab
          ...
          /* NO BADGE ANYWHERE IN THIS APP, and that is a decision rather than
             an omission. A badge is for a count somebody else is blocked on —
             Corvus badges friend requests because another person is waiting.
             Nothing here is like that: a queue length and a project count are
             the reader's own state, and badging them would train them to
             ignore the one place a badge might later matter. */
        />
      ))}
    </md-navigation-rail>
  );
 ...
 * an omission.
 *
 * Lyra puts one in the rail because posting is unambiguously its primary action
 * and it has a Create DESTINATION for the FAB to point at. Here the composer is
 * inline at the top of the feed — which is where this kind of app has always
 * put it, and which is why `route` has no `create()` to route to. A FAB would
 * therefore either duplicate a control that is already on screen, or point at a
 * screen that does not exist.
 *
 * `md-navigation-rail` renders nothing for an empty `slot="fab"`, so leaving it
 * out costs no layout.
 */
 ...
 * post and person drills are not destinations.
 *
 ...
 * because a real href is what makes middle-click and "copy link address"
 * behave. `originalEvent` is the MouseEvent or KeyboardEvent that produced the
 * selection, so one modifier check covers the Enter path too.
 *
 * THIS REPLACED A SINGLE BACK LINK. That link was an `<a>` styled as a link,
 * which is exactly what it looked like — a stray hyperlink floating above the
 * heading rather than part of the chrome. A trail is what the other three
 * verticals put in this row, and a reader moving between the four applications
 * should not find the fourth one navigating differently.
 */
      ...
      {crumbs.map((crumb, index) => (
        ...
        >
          {/* A crumb is either a translated label or a proper noun. The kit
              returns exactly one of the two and never a pre-translated
              string. */}
...
export function Screen({
   ...
   * noun, which only the screen knows.
   *
   * A screen that declared its own parent could disagree with the kit's route
   * table, and the two drill screens are exactly where that would go unnoticed.
   * `crumbsFor` returns an empty array on the five destinations, so that is
   * also how a top-level screen gets an empty row without saying anything.
   */
  const crumbs = crumbsFor(usePathname(), crumbLabel ?? null);

  return (
    <>
      {/* The row is ALWAYS rendered, even when empty.
          ...
          reserved in `.shell__trail`. Five of the seven screens leave it
          empty — they are destinations, and a trail naming only themselves
          would repeat the heading directly beneath it. */}
      ...
      {actions ? (
        <div className="screen-toolbar">
          ...
          <md-toolbar variant="floating" color="vibrant" aria-label={t('music.nav.toolbar')}>
            {actions}
          </md-toolbar>
        </div>
      ) : null}
      ...
      {/*
        ...
        mounted copies would give two `#social-explore-sort` triggers and the
  ...
  return (
    <>
      <div className="shell">
        ...
        {/* ABOVE THE ROUTER AND BELOW THE SCREEN. The bar is a sibling of the
            screen rather than a child, so a navigation replaces `children`
            and leaves this element — and everything it is playing — alone. */}
        <Transport />
 ...
 *
 * `hint` IS A STRING HERE, not the boolean the other verticals use.
 *
