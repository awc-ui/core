...
import '@awc-ui/showcase-kit/music/app.css';
import { ShowcaseProvider } from '@/lib/showcase';
import { PlayerProvider } from '@/lib/player';
import { SnackbarProvider } from '@/components/screens/Snackbar';
...
/*
 * `ShellProvider`, `PlayerProvider` AND `AppFrame` are all ABOVE the router,
 * for the same reason: `App` returns a different component per route, so
 * anything it renders is unmounted on every navigation.
 *
 * THE PLAYER PROVIDER IS THE ONE THIS VERTICAL EXISTS TO DEMONSTRATE. It holds
 * the transport — the loaded track, the playhead, the queue — and an app that
 * forgot those when you opened an album would not be a music app. It also holds
 * the mixer and the edit history, so a fader you moved on the Mixer screen is
 * still where you left it when you come back from the Studio.
 */
