...
@Component({
  selector: 'awc-root',
  standalone: true,
  imports: [TransportComponent, SnackbarHostComponent, RouterOutlet, AppBarComponent, RailComponent, BarComponent, DockComponent],
  ...
  template: `
    <div class="shell">
      ...
      <!-- ABOVE THE ROUTER AND BELOW THE SCREEN. "router-outlet" replaces
           the screen on every navigation and leaves this element — and
    ...
    <!-- LAST IN THE DOCUMENT, matching React, where the snackbar provider
         renders it after its children. It is "position: fixed", so where it
         sits makes no visual difference — but the parity census compares
         element ORDER. -->
    <awc-snackbar-host />
  `,
})
