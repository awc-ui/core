...
import { getViewer } from '@awc-ui/showcase-kit/music';
...
@Component({
  ...
  template: `
    <md-app-bar
      class="shell__appbar"
      variant="small"
      [attr.subtitle]="t('music.app.title')"
      [attr.leading-icon]="compactNav() ? null : 'menu'"
      [attr.leading-icon-label]="compactNav() ? null : t('music.nav.menu')"
      (mdLeadingClick)="shell.toggleRail()"
    >
      <span slot="headline" class="shell__brand">
        {{ t('music.app.brand') }}
        <md-tooltip [attr.text]="t('music.app.demoNotice')">
          <md-chip
            [attr.label]="t('music.app.demo')"
            appearance="outlined"
            color="warning"
            icon="science"
          ></md-chip>
      ...
      <md-avatar
        slot="trailing"
        [attr.src]="viewer.art.src"
        [attr.name]="viewer.displayName"
        [attr.label]="t('music.app.viewer', { name: viewer.displayName })"
        size="small"
      ></md-avatar>
    </md-app-bar>
  `,
