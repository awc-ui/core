...
function Breadcrumbs({ crumbs }: { crumbs: Crumb[] }) {
  ...
  //
  // The crumbs still carry real, fully-prefixed hrefs — not because they work
  // without JavaScript (this build renders nothing without it; that claim
  // belongs to the html and astro builds), but because a real href is what
  // makes ⌘-click, middle-click and "copy link address" behave. The deep link
  // it copies resolves on a cold load: the build fans `index.html` out across
  // every route.
  //
 ...
 *
 * It is a real anchor, which is why it honours ⌘-click and why the URL it
 * copies resolves on a cold load — but left alone it also means every nav click
 * was a FULL PAGE RELOAD in a client-routed app: measured as
 * `performance.navigation.type === 'navigate'` on each click, tearing down and
 * rebuilding the whole document, re-registering every component. That is what
 * made the top row lurch, and it costs more here than it did in the build this
 * was ported from — a reload of a single-page application re-runs the entire
 * bundle. The `mdClick` listener below vetoes the navigation and routes in
 * place instead; `preventDefault()` on `mdClick` is what gates it, not on the
 * native click.
 *
