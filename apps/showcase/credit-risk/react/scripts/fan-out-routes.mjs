 ...
 * RUNNING. A cold request for `/showcase/credit-risk/react/facilities/fac-001/`
 ...
 *
 * The usual fix is a host rewrite (`/showcase/credit-risk/react/* →
 * /showcase/credit-risk/react/index.html 200`). That rule is still worth having
 ...
 *      fallback, no rewrite hook. This build is the REFERENCE the other five are
 *      measured against, so five of its six parity routes 404ing is not one
 *      failure, it is thirty.
 ...
 * serves all 95.
 *
 * WHERE THE LIST COMES FROM. The same three fixture selectors that fed
 * `generateStaticParams()` in the Next build: `getSectors`, `getCounterparties`,
 * `getFacilities`. Add a row to the kit's fixture and its page appears with no
 * edit here — that property was the reason `generateStaticParams` was written
 * against the selectors rather than a literal list, and it is worth keeping.
 *
 * The route SHAPES come from the kit's `route` table, so they cannot drift from
 * the patterns `src/App.tsx` matches on.
 */
