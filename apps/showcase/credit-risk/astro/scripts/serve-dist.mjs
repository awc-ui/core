#!/usr/bin/env node
/**
 * Serve `dist/` at the real mount path — `/showcase/credit-risk/astro/` — so
 * the build can be checked exactly as it will be deployed. `astro preview`
 * serves at the configured base too, but this needs no Astro process and no
 * docs site: `pnpm verify` starts it itself, which is what makes
 * `pnpm showcase:verify` at the repo root a single command rather than a
 * sequence with a server to remember.
 *
...
const { basePath: BASE_PATH } = createRoutes('astro');
