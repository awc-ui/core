 ...
 *
 * The runtime is still needed on a server-rendered build, for a different
 * reason than it used to be. `src/hooks.server.ts` paints every component's
 * shadow DOM into the response, so the page is styled and readable before any
 * script runs; what the runtime adds is BEHAVIOUR — the tables sort, the charts
 * draw into their canvases, and the dock switches language, theme and density
 * in place. Whether it adopts the server's markup or re-renders over it depends
 * on annotations that Svelte's hydration may strip first; `hooks.server.ts`
