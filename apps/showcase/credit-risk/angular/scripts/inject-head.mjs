 ...
 * commentary before it is copied 95 times.
 *
 ...
 *
 * WHAT COMES OUT: every HTML comment. `src/index.html` carries about five
 * kilobytes of rationale — the ordering of the head, why the runtime is a plain
 * module script, why the body is empty — and all of it is for whoever edits that
 * file, none of it for the browser. Angular does not minify HTML, so without
 * this every visitor downloads the rationale, on every one of the 95 documents
 * `scripts/fan-out-routes.mjs` writes.
 *
 * Runs after `ng build` and before the fan-out, so the 95 copies are made from
 * the finished shell rather than 95 copies each needing the same edit.
 */
