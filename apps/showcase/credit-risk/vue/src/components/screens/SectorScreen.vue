 ...
 *
 * The Nuxt twin these screens were copied from cast the miss away and then
 * dereferenced it, so an unknown id threw inside the render — on a server,
 * where the blast radius of a throw is the one request that caused it. Here the
 * render is the browser's: a throw takes down the whole application, masthead
 * and dock included, and leaves a blank document with a stack trace in the
 * console. `src/App.vue` hands the captured segment straight down with nothing
 * checking it, so the lookup that decides whether this screen exists is this
 * one. Verified on `/sectors/banana/`: the empty state renders inside the shell
 * and the nav still works.
 */
const sector = computed(() => getSectorById(props.sectorId) as Sector | undefined);
...
const share = computed(() => sector.value?.portfolioShare ?? 0);
...
const crumbs = computed(() => [
  ...
  { label: sector.value ? t.value(sector.value.nameKey) : '' },
]);
</script>

<template>
  <MissingScreen v-if="!sector" />
