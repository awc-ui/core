...
import MissingScreen from './MissingScreen.vue';
...
const t = useT();
/*
 * `| undefined`, and in this build that branch is the LIVE path rather than a
 * guard against nothing. The Nuxt twin cast the miss away and then dereferenced
 * it, so an unknown id threw inside the render — on a server, where the blast
 * radius is the one request. Here the render is the browser's, so a throw takes
 * the whole application down — masthead and dock included — and `src/App.vue`
 * hands the captured segment straight down with nothing checking it first.
 *
 * Everything below reads through `facility.value?`, so the miss propagates as
 * empty rather than throwing on the way to the `v-if` in the template. Vue
 * evaluates a computed lazily, but `crumbs` and the chart props are read by the
 * `v-else` branch's own bindings, and a stray dereference in one of them would
 * only surface on the one URL nobody tests.
 */
const facility = computed(() => getFacilityById(props.facilityId) as Facility | undefined);
const counterparty = computed(() =>
  facility.value ? getCounterpartyById(facility.value.counterpartyId) : undefined,
);
const covenants = computed(() => (facility.value ? getCovenantsFor(facility.value.id) : []));
const collateral = computed(() => (facility.value ? getCollateralFor(facility.value.id) : []));
const schedule = computed(() => (facility.value ? drawdownSchedule(facility.value) : []));
const netCollateral = computed(() => collateral.value.reduce((sum, item) => sum + item.netValue, 0));
const coverage = computed(() =>
  facility.value && facility.value.ead > 0 ? netCollateral.value / facility.value.ead : 0,
);
