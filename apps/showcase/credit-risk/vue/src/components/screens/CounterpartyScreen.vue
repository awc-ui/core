<!--
  ...
  component hides the inactive ones — which is what we want: the rating history
  and the group tree are in the DOM, and therefore findable and reachable by
  assistive tech, before the visitor opens those tabs. (In the Nuxt twin the
  same sentence was about the prerendered HTML carrying them. Here there is no
  prerendered HTML; the reason to render all three is the same one either way.)
 ...
 */
const cp = computed(() => getCounterpartyById(props.counterpartyId) as Counterparty | undefined);
...
const tabsId = computed(() => `cp-tabs-${cp.value?.id ?? 'none'}`);
