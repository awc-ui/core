 ...
 * stages into `apps/docs/public/showcase/credit-risk/svelte/`.
 */
 ...
 * the component runtime is imported from. It is the build-time equivalent of the
 * `%awc.*%` placeholders the SvelteKit twin fills in per request.
 *
 ...
 *
 * `index.html` carries about five kilobytes of why — the head ordering, the
 ...
 * rationale, on every one of the 95 routes it is copied into. Svelte's own
 * markup comments never reach the output, so this is the only place the problem
 * arises.
 *
...
export default defineConfig({
  base: `${BASE_PATH}/`,
  plugins: [showcaseHead(), stripHtmlComments(), svelte()],
  resolve: {
    // The same `$lib` alias SvelteKit provided, so every screen, table and bit
    // copied out of the SSR twin keeps its import specifiers byte for byte.
  ...
  build: {
    ...
    // route would trade one request for six heavily overlapping ones, and each
