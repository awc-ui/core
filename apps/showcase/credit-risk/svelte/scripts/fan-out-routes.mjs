 ...
 * RUNNING. A cold request for `/showcase/credit-risk/svelte/facilities/fac-001/`
 ...
 *
 * The usual fix is a host rewrite (`/showcase/credit-risk/svelte/* →
 * /showcase/credit-risk/svelte/index.html 200`). That rule is still worth having
 ...
 *      fallback, no rewrite hook. Five of this build's six parity routes 404ing
 *      is not one failure, it is every comparison this build takes part in.
 ...
 * serves all 95.
 *
 * WHY 95 IDENTICAL FILES IS STILL AN SPA AND NOT A PRERENDER. Nothing about the
 * page is computed per route: no screen is rendered into any of them, and the
 * document served at `/facilities/fac-001/` is byte for byte the one served at
 * `/`. With JavaScript disabled every one of them is an empty `<div id="root">`.
 * What the fan-out replaces is the host's rewrite rule, not a render — which is
 * exactly the line between this build and the SvelteKit one in the same pair.
 *
 * WHERE THE LIST COMES FROM. The same three fixture selectors that fed the
 * `entries()` exports the SvelteKit twin used while it prerendered:
 * `getSectors`, `getCounterparties`, `getFacilities`. Add a row to the kit's
 * fixture and its page appears with no edit here — that property is the reason
 * the enumeration was written against selectors rather than a literal list, and
 * it is worth keeping.
 *
