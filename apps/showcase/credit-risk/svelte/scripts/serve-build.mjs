#!/usr/bin/env node
/**
 * Serve `build/` at the real mount path — `/showcase/credit-risk/svelte/` — so
 * the prerendered output can be checked exactly as it will be deployed. `vite
 * preview` serves at `/` and every asset URL in the output carries the base
 * path, so it 404s on all of them.
 *
...
createServer((req, res) => {
  ...
  // resolves inside `build/` rather than above it.
