 ...
 * already in a language. So — exactly like the Astro build, and for exactly the
 * same reason — the language is part of the URL and switching it is a
 * navigation:
 *
 *   /showcase/credit-risk/html/           en   (default, unprefixed)
 *   /showcase/credit-risk/html/ro/        ro
 *   /showcase/credit-risk/html/ar/        ar   (dir="rtl")
 *
...
/**
 * A callable translator: `t('table.ead')` rather than `t.t('table.ead')`, with
 * the locale-bound `Intl` formatters hanging off the same value.
 *
 * The screens call this hundreds of times, and keeping the shape identical to
 * the React and Astro builds is what lets a screen be read side by side with
 * its twin in another framework and differ only where the framework differs.
 */
const cache = new Map();
