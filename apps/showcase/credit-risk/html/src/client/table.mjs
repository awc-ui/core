...
const INITIAL = { column: 'ead', order: 'desc' };
const PAGE_SIZE = 10;
...
export function enhancePagedTables(root = document) {
  for (const table of root.querySelectorAll('md-table[data-paged]')) {
    ...
    const template = container?.parentElement?.querySelector('template[data-rows]');
    ...
    table.addEventListener('mdSortChange', (event) => {
      ...
      sort = !column || order === 'none' ? { ...INITIAL } : { column, order };
      render();
    });

    pagination?.addEventListener('mdPageChange', (event) => {
      page = event.detail?.page ?? 0;
      render();
    });
