 ...
 * app at all; it is copied here and served as a plain static file from an
 * absolute URL — the same thing the docs site does.
 *
 * WHAT IT IS STILL FOR, NOW THAT THE SERVER PAINTS THE COMPONENTS.
 * `src/server.ts` gives every `md-*` element its declarative shadow root before
 * the response leaves the process, so the page arrives painted rather than as a
 * column of unstyled tags. That is the FIRST FRAME. The runtime is what makes
 * the components INTERACTIVE — it adopts those shadow roots, draws the charts
 * into their canvases, and answers the dock when the language, theme or density
 * changes in place. Server rendering does not remove it from the browser's
 * critical path; it removes the wait from what the reader sees.
 *
