  ...
  return (
    ...
    >
      {/*
        ...
        the instruments, the account holder — and it is all presented in the
        shape of a real banking app, which is exactly the combination a
      ...
      */}
      ...
      {/* A consumer app has no reporting quarter — the meaningful frame is the
          statement period and the currency the roll-ups are shown in. */}
      ...
      */}
      ...
      {DESTINATIONS.map((destination) => (
        <md-navigation-rail-tab
          ...
          /* The badge counts what is still settling. It sits on the
             statement because that is where a reader goes to look at a
             pending row — a count on a destination that cannot show the
             thing it counts is a dead end. */
          badge-value={
 ...
 * rail's own.
 *
 * IT ROUTES TO THE EXCHANGE SCREEN, which is a real destination with a real
 * ticket on it — moving money between currencies is this app's primary action
 * and it already has a home, so unlike the wealth console's FAB there is
 * nothing placeholder about this wiring.
 *
...
function Fab() {
  ...
  useCustomEvent<CustomEvent>(ref, 'mdClick', () => router.push(route.exchange()));

  return <md-fab ref={ref} slot="fab" icon="currency_exchange" label={t('banking.action.exchange')} />;
}
...
function Bar() {
  ...
  return (
    <md-navigation-bar
      ref={ref}
      class="shell__bar"
      aria-label={t('banking.nav.label')}
      active-index={activeIndex}
...
function Breadcrumbs({ crumbs }: { crumbs: CrumbSpec[] }) {
  ...
  return (
    <md-breadcrumbs
      ref={ref}
      label={t('banking.nav.breadcrumb')}
      max-items="4"
    ...
    <>
      ...
      {actions ? (
        <div className="screen-toolbar">
          ...
          <md-toolbar variant="floating" color="vibrant" aria-label={t('banking.nav.toolbar')}>
            {actions}
          </md-toolbar>
        </div>
      ) : null}
      ...
      {/*
        ...
        mounted copies would give two `#banking-holdings-export` triggers and the
...
export function EmptyState({ message, hint = false }: { message: string; hint?: boolean }) {
  ...
  return (
    <div className="empty">
      <p>{message}</p>
      {hint ? <p>{t('banking.empty.hint')}</p> : null}
    </div>
  );
}
