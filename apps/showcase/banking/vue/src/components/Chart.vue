...
<template>
  <component
    ...
    :label-plot="t('banking.chart.plotHint')"
  >
    <!-- Forwarded so a donut can fill its `center` slot. The React twin gets
         this for free — `children` rides through the props spread — but a Vue
         component renders nothing it is not told to. -->
    <slot />
  </component>
</template>
