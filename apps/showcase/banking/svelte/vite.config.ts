 ...
 * `/showcase/banking/<framework>/`, so every asset URL, every link and the
 ...
 * being loaded — so both read `SHOWCASE_BASE` from `@awc-ui/showcase-kit/banking`
 ...
 * stages into `apps/docs/public/showcase/banking/svelte/`.
 */
...
import { FRAMEWORKS, REPORTING_DATE, SHOWCASE_BASE } from '@awc-ui/showcase-kit/banking';
...
function showcaseHead(): Plugin {
  const tokens: Record<string, string> = {
    ...
    __AWC_TITLE__: `${en['banking.app.brand']} — ${en['banking.app.title']}`,
    __AWC_DESCRIPTION__: en['banking.app.subtitle'],
