 ...
 * fills.
 *
 * NOTE WHILE THE SCREENS ARE STUBS: sections [1]–[3], [5a], [6] and [8] hold
 * against the scaffold (the chrome, the router and the dock are real); the
 * drill assertion in [5] and the whole of [7] exercise the ported screens
 * (the overview book table's drill anchors, the holdings positions table) and
 * pass only once the screens phase lands.
 *
 * Starts its own server, so it needs nothing running:
 *   pnpm --filter @awc-ui/showcase-banking-angular build
 *   pnpm --filter @awc-ui/showcase-banking-angular verify
 *
...
import { createRoutes, FRAMEWORKS } from '@awc-ui/showcase-kit/banking';
...
/* ------------------------------- 2. the 13 documents are the same document -- */
{
  ...
  const paths = [route.overview(), route.holdings(), route.proposals(), route.household('hh-01'), route.planning()];
  const bodies = [];
...
/* ----------------------------------------- 3. cold deep links reach the screen */
{
  ...
  const paths = [route.overview(), route.holdings(), route.proposals(), route.trade(), route.planning(), route.household('hh-01')];
  for (const path of paths) {
...

{
  ...
  ok('the URL moved to the holdings', where.path === `${basePath}${appPath(route.holdings())}`, where.path);
  ok('the holdings screen is on screen', where.heading !== '(none)', where.heading);
  ok('the rail marks the destination it is on', where.activeIndex === '1', `active-index=${where.activeIndex}`);

  // Back to the overview through the rail (a third history entry), then a
  // drill anchor — `<a class="drill" routerLink>` in the overview's book
  // table. SCREENS PHASE: the drill assertions pass once OverviewBookTable is
  // ported.
  await page.evaluate(() => {
    const tab = document.querySelector('md-navigation-rail-tab[value="overview"]');
    (tab.shadowRoot?.querySelector('a, button') ?? tab).click();
  });
  await new Promise((r) => setTimeout(r, 1500));
  const drilled = await page.evaluate(() => {
    const drill = document.querySelector('a.drill');
  ...
  ok('a drill anchor routes without a reload', drilled && probe.stamp === 'alive' && probe.navigations === 1, drilled ? `${probe.stamp}, ${probe.navigations} navigation(s)` : '(no a.drill on the overview — screens not ported yet)');
  ok('it drilled into a household', /\/households\/[^/]+$/.test(drillWhere.path), drillWhere.path);
  ok('and the trail appeared', drillWhere.crumbs > 1, `${drillWhere.crumbs} crumb(s)`);

  // Back, to the overview entry the rail click wrote.
  await page.goBack();
  await new Promise((r) => setTimeout(r, 1500));
  probe = await readStamp(page);
...
/* ------------------------------------------------------------ 6. the locale */
{
  ...
  ok('and the screen is actually in Romanian', /Prezentare|Dețineri|Propuneri|portofoliu/i.test(probe.text), probe.text.slice(0, 48).replace(/\s+/g, ' '));
 ...
 * component. SCREENS PHASE: this section drives the holdings positions table
 * (66 rows, paginated 25 a page) and passes once HoldingsTables is ported.
 */
{
  console.log('\n[7] the table sorts and pages through the selector');
  const page = await load(`${BASE}${route.holdings()}`);
  ...
  const sorted = await page.evaluate(`(async () => {
    ...
    document.querySelector('md-table')?.dispatchEvent(
      new CustomEvent('mdSortChange', { detail: { column: 'ticker', order: 'asc' }, bubbles: true }),
    );
  ...
  ok(
    ...
    sorted.after.length > 0 && sorted.after.join() !== sorted.before.join() && sorted.after.every(Boolean),
    `${sorted.before[0]} → ${sorted.after[0]}`,
  );
  ok(
    'paging moves the slice',
    paged.after.length > 0 && paged.after.every(Boolean) && paged.after.every((id) => !paged.before.includes(id)),
...
/* -------------------------------------------------------------- 8. the dock */
{
  ...
  ok('the dock is labelled for the Vela app', !!probe.label, probe.label ?? '(none)');
