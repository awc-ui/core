...
function appBar(t) {
  ...
  return html`<md-app-bar${attrs({
    ...
    <span slot="headline" class="shell__brand">${t('banking.app.brand')}<md-tooltip${attrs({
      text: t('banking.app.demoNotice'),
    })}><md-chip${attrs({
        ...
        ${actions
          ? html`<div class="screen-toolbar">
              <md-toolbar${attrs({
                variant: 'floating',
                color: 'vibrant',
                'aria-label': t('banking.nav.toolbar'),
              })}>${actions}</md-toolbar>
            </div>`
          : null}
...
export function emptyState(t, message, { hint = false, attributes = {} } = {}) {
  return html`<div class="empty"${attrs(attributes)}>
    <p>${message}</p>
    ${hint ? html`<p>${t('banking.empty.hint')}</p>` : null}
  </div>`;
}
