 ...
 *   pnpm --filter @awc-ui/showcase-community-vue build
 *   pnpm --filter @awc-ui/showcase-community-vue verify
 *
...
/* --------------------------------------------- the document really is a shell */
{
  ...
  for (const path of [route.feed(), route.post('pst-04')]) {
  ...
  const [a, b] = await Promise.all([
    ...
    fetch(`${BASE}${route.post('pst-04')}`).then((r) => r.text()),
  ]);
...
/* ---------------------------------------------------- routing in the browser */
{
   ...
   *
   * Friends is the target because it is the second destination, so the
  ...
  // that: a post's parent is the feed, a person's is friends).
  await page.goto(`${BASE}${route.post('pst-04')}`, { waitUntil: 'networkidle0', timeout: 90000 });
  ...
  ok('the back button restores the previous screen', /\/p\/pst-04\/$/.test(where.path), where.path);
...
/* --------------------------------------------- the state query survives a hop */
{
  ...
  await page.waitForFunction(
    () => Boolean(document.querySelector('md-navigation-rail-tab[value="groups"]')?.shadowRoot?.querySelector('a, button')),
    { timeout: 20000 },
  );
  await page.evaluate(() => {
    document.querySelector('md-navigation-rail-tab[value="groups"]').shadowRoot.querySelector('a, button').click();
  });
