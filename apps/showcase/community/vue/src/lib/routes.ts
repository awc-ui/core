 ...
 *
 * The screen map itself lives in `@awc-ui/showcase-kit/community` and is identical
 * across every framework build. The only local fact is WHICH framework this
 * build is, and that single fact has to agree with `base` in `vite.config.ts` —
 * so it is declared once, here, and everything else is derived.
 *
...
export const { framework: FRAMEWORK, basePath: BASE_PATH, withBase } = routes;
