  ...
  template: `
    ...
    @if (hasActions) {
      <div class="screen-toolbar">
        ...
        <md-toolbar variant="floating" color="vibrant" [attr.aria-label]="t('community.nav.toolbar')">
...
export class ScreenComponent extends ShowcaseComponent implements OnInit, OnDestroy {
   ...
   */
  @Input() crumbLabel: string | null = null;
  ...
  protected get crumbs(): CrumbSpec[] {
    /* Angular's `url` has no trailing slash and the kit's route table is all
       slashed — the same mismatch `lib/routes.ts` documents for links, handled
       here in the one place that reads the current path. */
    const path = this.router.url.split('?')[0];
    return crumbsFor(path.endsWith('/') ? path : `${path}/`, this.crumbLabel);
  }
