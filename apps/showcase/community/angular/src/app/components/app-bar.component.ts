...
@Component({
  ...
  template: `
    <md-app-bar
      class="shell__appbar"
      variant="small"
      [attr.subtitle]="t('community.app.title')"
      [attr.leading-icon]="compactNav() ? null : 'menu'"
      [attr.leading-icon-label]="compactNav() ? null : t('community.nav.menu')"
      (mdLeadingClick)="shell.toggleRail()"
    >
      <span slot="headline" class="shell__brand">
        {{ t('community.app.brand') }}
        <md-tooltip [attr.text]="t('community.app.demoNotice')">
          <md-chip
            [attr.label]="t('community.app.demo')"
            appearance="outlined"
            color="warning"
            icon="science"
          ></md-chip>
      ...
      <md-avatar
        ...
        [attr.label]="t('community.app.viewer', { name: viewer.displayName })"
        size="small"
      ></md-avatar>
    </md-app-bar>
  `,
