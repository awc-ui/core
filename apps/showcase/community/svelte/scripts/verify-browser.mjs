 ...
 *   pnpm --filter @awc-ui/showcase-community-svelte build
 *   pnpm --filter @awc-ui/showcase-community-svelte verify
 *
...
import { createRoutes, DESTINATIONS, FRAMEWORKS } from '@awc-ui/showcase-kit/community';
 ...
 */
{
  ...
  const deep = await (await fetch(`${BASE}${route.post('pst-04')}`)).text();
  ...
  await page.goto(`${BASE}${route.friends()}`, { waitUntil: 'domcontentloaded' });
 ...
 * (`/showcase/community/svelte/* → …/index.html 200`) and the only condition
...
/* ---------------------------------------------------- routing in the browser */
{
  ...
  ok('the URL moved to friends', where.path === `${basePath}${route.friends()}`, where.path);
  ok('the friends screen is on screen', where.heading.length > 0 && where.path.endsWith(route.friends()), where.heading);
  ok(
    'the rail marks the destination it is on',
    where.activeIndex === DESTINATIONS.findIndex((d) => d.value === 'friends'),
    `active-index=${where.activeIndex}`,
  );

  /* Back to the feed, then a drill anchor. This vertical has no `<a
     class="drill">` — the anchor that always leads to a post is
     `.post-media__link`. Same claim either way: an in-app anchor routes rather
     than reloading. */
  await page.goBack();
  await new Promise((r) => setTimeout(r, 1500));
  await page.evaluate(() => document.querySelector('a.when').click());
  await new Promise((r) => setTimeout(r, 1500));
  ...
  ok('it drilled into a post', /\/p\/pst-\d+\/$/.test(where.path), where.path);
