 ...
 *      — `/showcase/community/react/assets/…` here — where nothing was ever
...
/*
 * The preboot script, as a FILE — it used to be inlined into index.html, which
 * an enterprise `script-src 'self'` refuses. The kit already builds it
 * standalone; copying it beside the runtime lets the document reference it by
 * URL and carry no executable text of its own. It stays a blocking classic
 * script in the head: its job is to stamp theme/density/dir before first paint,
 * and the parser still waits for an external one.
 */
const prebootSource = resolve(repoRoot, 'packages/showcase-kit/dist/preboot.js');
if (!existsSync(prebootSource)) {
  console.error(`[sync-runtime] ${prebootSource} is missing — build the kit first.`);
  process.exit(1);
}
cpSync(prebootSource, resolve(appRoot, 'public/preboot.js'));
