<!--
  ...
  to intercept. It routes to Create, which is also a destination in the rail
  below it — deliberately: posting is the primary action so M3 wants it on the
  FAB, but it also has to be addressable and present in the compact bar, which
  has no FAB slot. One control cannot be both. It is a real destination with a
  real ticket on it, unlike
  the React source does; the label already names the real action.
-->
<script setup lang="ts">
import { computed } from 'vue';
import { getTotals } from '@awc-ui/showcase-kit/social';
...
const fabListeners = { mdClick: () => router.push(route.create()) };
</script>
