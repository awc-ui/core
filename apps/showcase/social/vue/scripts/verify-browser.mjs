 ...
 *   pnpm --filter @awc-ui/showcase-social-vue verify
 *
...
/* --------------------------------------------- the document really is a shell */
{
  ...
  for (const path of [route.feed(), route.post('post-02')]) {
  ...
  const [a, b] = await Promise.all([
    fetch(`${BASE}${route.feed()}`).then((r) => r.text()),
    fetch(`${BASE}${route.post('post-02')}`).then((r) => r.text()),
  ]);
  ok('every route is byte-identical to the feed', a === b, a === b ? 'identical' : `${a.length} vs ${b.length} bytes`);
}
...
/* ---------------------------------------------------- routing in the browser */
{
   ...
   *
   * Explore is the target because it is the second destination, so the
   * indicator has somewhere to move TO — routing to the destination you are
   * already on would assert nothing about the indicator.
   *
  ...
  // `mdSelect` on the strip. A post deep link renders a two-crumb trail, and
  // the first crumb routes back to the feed (`crumbsFor` in the kit decides
  // that: a post's parent is the feed, a person's is explore).
  await page.goto(`${BASE}${route.post('post-02')}`, { waitUntil: 'networkidle0', timeout: 90000 });
  ...
  ok('it went back up to the feed', where.path === `${basePath}/`, where.path);

  // Back restores the post screen without a reload.
  await page.goBack();
  await new Promise((r) => setTimeout(r, 1500));
  probe = await readStamp(page);
  where = await page.evaluate(() => ({ path: location.pathname, crumbs: document.querySelectorAll('md-breadcrumb-item').length }));
  ok('the back button restores the previous screen', /\/p\/post-02\/$/.test(where.path), where.path);
...
/* ------------------------------------------------------------ locale in place */
{
  ...
  /* Words from THIS vertical's Romanian dictionary, and ones with a diacritic
     among them: an English string would pass a test that only looked for
     "Explore" spelled the same way in both. */
...
/* --------------------------------------------- the state query survives a hop */
{
  ...
  await page.waitForFunction(
    () => Boolean(document.querySelector('md-navigation-rail-tab[value="create"]')?.shadowRoot?.querySelector('a, button')),
    { timeout: 20000 },
  );
  await page.evaluate(() => {
    document.querySelector('md-navigation-rail-tab[value="create"]').shadowRoot.querySelector('a, button').click();
  });
...
/* ------------------------------------------------------------------- the dock */
{
  ...
  // Console" under a Vela app. DockBar.vue passes `label`; this catches
