...
import { DESTINATIONS, getTotals, getViewer, type CrumbSpec } from '@awc-ui/showcase-kit/social';
      ...
      */}
      ...
      {DESTINATIONS.map((destination) => (
        <md-navigation-rail-tab
          ...
          /* The badge counts what has not been read. It sits on Activity
             because that is the screen that shows the unread rows — a count
             on a destination that cannot show the thing it counts is a dead
             end. Two digits by design: a badge that has to hold "10" is the
             one that gets clipped by its host's corner, so that is the case
             the fixture makes sure exists. */
          badge-value={
            destination.value === 'activity' && totals.unreadActivityCount > 0
              ? String(totals.unreadActivityCount)
              : undefined
          }
        />
      ))}
 ...
 * rail's own.
 *
 * IT ROUTES TO CREATE, which is also a destination in the rail below it, and
 * that duplication is deliberate rather than overlooked.
 *
 * Posting is unambiguously this app's primary action, so M3 wants it on the
 * FAB; but it also has to be ADDRESSABLE — a real path, in the same keyboard
 * list as its four siblings, and present in the compact navigation bar, which
 * has no FAB slot at all. One control satisfies neither requirement. Every app
 * of this shape ends up with both for the same reason.
 *
...
function Fab() {
  ...
  useCustomEvent<CustomEvent>(ref, 'mdClick', () => router.push(route.create()));

  return <md-fab ref={ref} slot="fab" icon="add_a_photo" label={t('social.screen.create.title')} />;
}
