 ...
 * `/showcase/social/<framework>/`, so every asset URL, every link and the
 ...
 * being loaded — so both read `SHOWCASE_BASE` from `@awc-ui/showcase-kit/social`
...
import { FRAMEWORKS, REPORTING_DATE, SHOWCASE_BASE } from '@awc-ui/showcase-kit/social';
...
function showcaseHead(): Plugin {
  const tokens: Record<string, string> = {
    ...
    __AWC_TITLE__: `${en['social.app.brand']} — ${en['social.app.title']}`,
    __AWC_DESCRIPTION__: en['social.screen.feed.subtitle'],
...
export default defineConfig({
  ...
  server: {
    // Twenty above the banking build's 4347, which is twenty above credit-risk's
    // 4327 — one band per vertical, so every build of every vertical can run at
