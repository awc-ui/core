...
function appBar(t) {
  ...
  return html`<md-app-bar${attrs({
    ...
    'leading-icon': 'menu',
    'leading-icon-label': t('social.nav.menu'),
  })}>
    <span slot="headline" class="shell__brand">${t('social.app.brand')}<md-tooltip${attrs({
      text: t('social.app.demoNotice'),
    })}><md-chip${attrs({
    ...
    <md-avatar slot="trailing"${attrs({
      ...
      label: t('social.app.viewer', { name: profile.displayName }),
      size: 'small',
    })}></md-avatar>
  </md-app-bar>`;
}
 ...
 * page's own path.
 *
 * The FAB has no `href` prop, so its action (open the exchange screen) is
...
function rail(t, locale, here) {
  ...
  return html`<md-navigation-rail${attrs({
    ...
    <md-fab slot="fab"${attrs({
      ...
      'data-fab-href': localeHref(locale, route.create()),
    })}></md-fab>
        ...
        ${actions
          ? html`<div class="screen-toolbar">
              <md-toolbar${attrs({
                variant: 'floating',
                color: 'vibrant',
                'aria-label': t('social.nav.toolbar'),
              })}>${actions}</md-toolbar>
            </div>`
          : null}
