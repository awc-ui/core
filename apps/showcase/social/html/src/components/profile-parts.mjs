...
import { postKindIcon, route } from '@awc-ui/showcase-kit/social';
  ...
  return panel({
    children: html`<div class="profile-head">
        ...
        <div class="profile-head__text">
          ...
          <dl class="stat-row">
            <div><dt>${t('social.count.posts')}</dt><dd>${count(t, posts.length, { exact: true })}</dd></div>
            <div><dt>${t('social.count.followers')}</dt><dd>${count(t, person.followerCount, { exact: true })}</dd></div>
            <div><dt>${t('social.count.following')}</dt><dd>${count(t, person.followingCount, { exact: true })}</dd></div>
            <div><dt>${t('social.count.likes')}</dt><dd>${count(t, likes)}</dd></div>
          </dl>
