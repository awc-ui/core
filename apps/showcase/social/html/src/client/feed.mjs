import { matchesDiscovery } from '@awc-ui/showcase-kit/social';
...
export function enhanceFeed(root = document, onCloned = () => {}) {
  ...
  const render = () => {
    const matches = cards.filter((card) =>
      matchesDiscovery(
        ...
        card.querySelector('.post-actions__save')?.hasAttribute('data-on') ?? false,
      ),
    );
  ...
  host.addEventListener('mdClick', (event) => {
    if (filter === 'saved' && event.target?.closest?.('.post-actions__save')) render();
  });
}
