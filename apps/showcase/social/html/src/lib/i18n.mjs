 ...
 * a navigation:
 *
 *   /showcase/social/html/           en   (default, unprefixed)
 *   /showcase/social/html/ro/        ro
 *   /showcase/social/html/ar/        ar   (dir="rtl")
 *
...
import { createRoutes } from '@awc-ui/showcase-kit/social';
...
/**
 * A callable translator: `t('social.table.drift')` rather than
 * `t.t('social.table.drift')`, with the locale-bound `Intl` formatters hanging
 * off the same value.
 *
...
export function useT(locale) {
  ...
  const callable = Object.assign(fn, {
    ...
    /* BOTH INSTANTS ARE ARGUMENTS. The kit's signature takes the reference
       instant rather than reading the clock, and this build is the one where
       that matters most: a page is written once, at build time, and shipped —
       a "3h ago" computed against `Date.now()` would freeze the build machine's
       afternoon into the file and be wrong by the time anyone read it. Every
       caller passes REPORTING_INSTANT, so the five builds agree. */
    formatRelativeTime: tr.formatRelativeTime.bind(tr),
    locale,
    dir: dirFor(locale),
  });
