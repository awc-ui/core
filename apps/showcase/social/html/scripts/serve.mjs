#!/usr/bin/env node
/**
 * Serve `dist/` at the real mount path — `/showcase/social/html/` — so the
 ...
 *
 * The default port is 4352 — the credit-risk HTML build's 4322 plus the +10 a
 * vertical adds throughout, three verticals along, so all four can serve at
 * once. (Which they could not until this was written: banking's copy of this
 * file still had wealth's 4332 and the second one to start died on
 * EADDRINUSE.)
 *
...
const port = Number(process.argv[2] || 4352);
