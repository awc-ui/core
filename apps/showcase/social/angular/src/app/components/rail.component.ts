...
export class RailComponent extends ShowcaseComponent {
  ...
  protected badgeFor(destination: Destination): string | null {
    ...
    return destination.value === 'activity' && this.totals.unreadActivityCount > 0
      ? String(this.totals.unreadActivityCount)
      : null;
  }
  ...
  protected newProposal(): void {
    void this.router.navigateByUrl(this.appPath(this.route.create()));
  }
}
