 ...
 *   pnpm --filter @awc-ui/showcase-social-angular build
 *   pnpm --filter @awc-ui/showcase-social-angular verify
 *
...
import { createRoutes, FRAMEWORKS } from '@awc-ui/showcase-kit/social';
...
/* ------------------------------- 2. the 13 documents are the same document -- */
{
  ...
  const paths = [route.feed(), route.explore(), route.create(), route.post('post-02'), route.profile()];
  const bodies = [];
...
/* ----------------------------------------- 3. cold deep links reach the screen */
{
  ...
  const paths = [route.feed(), route.explore(), route.create(), route.activity(), route.profile(), route.post('post-02'), route.person('ada.lind')];
  for (const path of paths) {
...

{
  ...
  // the native click.
  await page.evaluate(() => {
    const tab = document.querySelector('md-navigation-rail-tab[value="explore"]');
  ...
  ok('the URL moved to explore', where.path === `${basePath}${appPath(route.explore())}`, where.path);
  ok('the explore screen is on screen', where.heading !== '(none)', where.heading);
  ...
  const drilled = await page.evaluate(() => {
    const drill = document.querySelector('a.post-media__link');
  ...
  ok('it drilled into a post', /\/p\/post-[^/]+$/.test(drillWhere.path), drillWhere.path);
...
/* ------------------------------------------------------------ 6. the locale */
{
  ...
  ok('and the screen is actually in Romanian', /Explorează|urmărești|Activitate/i.test(probe.text), probe.text.slice(0, 48).replace(/\s+/g, ' '));
...
/* -------------------------------------------------- 7. the profile's tabs */
/*
 * THIS SLOT HELD A TABLE SUITE — sorting and paging a 66-row holdings table
 * through the kit's selector — and there is no table anywhere in this vertical
 * to point it at. It is pictures, names and times.
 *
 * What replaces it is the one place this app switches sibling views inside a
 * single screen, which is the closest thing it has to that shape and is the one
 * legitimate use of `md-tabs` in the whole application: the profile's posts,
 * saved and tagged. The claim is the same in spirit — a component event drives
 * a re-render from state rather than the component quietly managing its own.
 */
{
  console.log('\n[7] the profile switches tabs from state');
  const page = await load(`${BASE}${route.profile()}`);

  const before = await page.evaluate(() => ({
    tabs: document.querySelectorAll('md-tab').length,
    cells: document.querySelectorAll('.post-grid__cell').length,
    empty: document.querySelectorAll('.empty').length,
  }));

  await page.evaluate(async () => {
    document.querySelector('md-tabs')?.dispatchEvent(
      new CustomEvent('mdTabChange', { detail: { value: 'tagged' }, bubbles: true, composed: true }),
    );
  ...
  ok('three tabs over one screen', before.tabs === 3, `${before.tabs} tabs`);
  ok('the posts tab opens with a grid', before.cells > 0 && before.empty === 0, `${before.cells} tiles`);
  /* Tagged is deliberately empty — nothing in the fixture models "tagged in",
     and inventing a field for one tab would be data added to serve a layout. */
  ok(
    'switching to tagged swaps the grid for its empty state',
    after.cells === 0 && after.empty === 1,
    `${after.cells} tiles, ${after.empty} empty state(s)`,
  );
  await page.close();
}
