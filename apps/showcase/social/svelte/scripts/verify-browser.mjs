 ...
 *   pnpm --filter @awc-ui/showcase-social-svelte build
 *   pnpm --filter @awc-ui/showcase-social-svelte verify
 *
...
import { createRoutes, DESTINATIONS, FRAMEWORKS } from '@awc-ui/showcase-kit/social';
 ...
 */
{
  ...
  const deep = await (await fetch(`${BASE}${route.post('post-02')}`)).text();
  ...
  await page.goto(`${BASE}${route.explore()}`, { waitUntil: 'domcontentloaded' });
...
/*
 * `PersonScreen` guards its own handle — a component taking a plain string from
 * a URL must not trust its caller — and renders the empty state for an id the
 * fixture does not know.
 *
 * THE SHELL HAS TO BE SERVED FOR THE APP TO GET THE CHANCE. `/people/nobody-at-all/`
 ...
 * (`/showcase/social/svelte/* → …/index.html 200`) and the only condition
 ...
 */
{
  ...
  page.on('request', (request) => {
    const isRewrite =
      ...
      new URL(request.url()).pathname === `${basePath}/people/nobody-at-all/`;
    if (isRewrite) request.respond({ status: 200, contentType: 'text/html; charset=utf-8', body: shell });
    else request.continue();
  });

  await page.goto(`${BASE}/people/nobody-at-all/`, { waitUntil: 'networkidle0', timeout: 90000 });
  ...
  const probe = await page.evaluate(() => ({
    ...
    heading: document.querySelector('.screen-head h1')?.textContent ?? '(none)',
    /* The feed owns `/`; if the lookup guard were missing this would either
       throw or fall through to a screen that is not the empty state. Asserted
       on the POSTS rather than on the heading text, which would have to be
       spelled out in one locale to compare against. */
    isFeed: Boolean(document.querySelector('.post-card')),
    empty: Boolean(document.querySelector('.empty')),
  }));
  ok('it renders the empty state rather than throwing', probe.shell && probe.empty && !probe.isFeed, probe.heading);
  ok(
...
/* ---------------------------------------------------- routing in the browser */
{
  ...
  ok('the URL moved to explore', where.path === `${basePath}${route.explore()}`, where.path);
  ok('the explore screen is on screen', where.heading.length > 0 && where.path.endsWith(route.explore()), where.heading);
  ok(
    'the rail marks the destination it is on',
    where.activeIndex === DESTINATIONS.findIndex((d) => d.value === 'explore'),
    `active-index=${where.activeIndex}`,
  );

  /* Back to the feed, then a drill anchor. This vertical has no `<a
     class="drill">` — its drills are pictures, and the anchor that wraps one is
     `.post-media__link`. Same claim either way: an in-app anchor routes rather
     than reloading. */
  await page.goBack();
  await new Promise((r) => setTimeout(r, 1500));
  await page.evaluate(() => document.querySelector('a.post-media__link').click());
  await new Promise((r) => setTimeout(r, 1500));
  ...
  ok('it drilled into a post', /\/p\/post-\d+\/$/.test(where.path), where.path);
  ok('and the trail appeared', where.crumbs > 1, `${where.crumbs} crumb(s)`);

  // Back, all the way to the feed.
  await page.goBack();
  await new Promise((r) => setTimeout(r, 1500));
  probe = await readStamp(page);
 ...
 */
{
  ...
  /* `.post-media__link`, not `a.drill`: this vertical's drills are pictures.
     The claim is unchanged — an in-app anchor carries the dock's query with
     it. */
  await page.evaluate(() => document.querySelector('a.post-media__link').click());
...
/* ------------------------------------------------------------------- the dock */
{
  ...
  ok('it is labelled as the Lyra app', Boolean(probe.label) && !/credit/i.test(String(probe.label)), String(probe.label ?? '(none)'));
