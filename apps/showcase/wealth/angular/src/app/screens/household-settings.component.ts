...
import { ActionButtonComponent } from './household-tabs.component';
 ...
 *     emits nothing when `selected` is assigned from script — or, here, when
 *     Angular rewrites the attribute. So the flip cannot re-enter the handler,
 *     and a controlled `selected` is safe.
 ...
 *     Escape alike — is what puts the state back. It neither bubbles nor
 *     composes, so the listener sits on the sheet element itself.
 *
 ...
 * which one moved. Four template refs and four handlers for four switches is
 * the version of this that rots.
 */

/**
 * What the reader has chosen to see. Pure view state — nothing here is a domain
 * value, which is exactly why it lives in the app and not in the kit.
 */
...
@Component({
  selector: 'awc-household-settings',
  standalone: true,
  ...
  template: `
    <md-side-sheet
      ...
      variant="modal"
      side="end"
      [attr.headline]="t('wealth.action.filter')"
      top-divider
      (mdClose)="closed.emit()"
    >
      <div class="row row--end">
        <md-button awcAction icon="restart_alt" (activate)="viewChange.emit(defaults)">
      ...
      <md-list
        class="table-host"
        [attr.label]="t('wealth.action.filter')"
        interaction-mode="multi-action"
        (mdChange)="onToggle($event)"
      >
        @for (toggle of toggles; track toggle.key) {
          <md-list-item
            [attr.headline]="t(toggle.labelKey)"
            [attr.leading-icon]="toggle.icon"
            lines="1"
          >
            <md-switch
              slot="trailing"
              [attr.data-key]="toggle.key"
              [attr.selected]="view[toggle.key] ? '' : null"
              [attr.aria-label]="t(toggle.labelKey)"
            ></md-switch>
          </md-list-item>
        }
      </md-list>
...
export class HouseholdSettingsSheetComponent extends ShowcaseComponent {
  @Input({ required: true }) open!: boolean;
  @Input({ required: true }) view!: HouseholdView;
  @Output() readonly viewChange = new EventEmitter<HouseholdView>();
  @Output() readonly closed = new EventEmitter<void>();

  protected readonly toggles = TOGGLES;
  protected readonly defaults = DEFAULT_VIEW;

  protected onToggle(event: Event): void {
    const key = (event.target as HTMLElement | null)?.dataset?.['key'] as
      | keyof HouseholdView
      | undefined;
    if (!key) return;
