...
import { getBookTotals } from '@awc-ui/showcase-kit/wealth';
 ...
 * proposals screen as a placeholder for the real raise-proposal flow, exactly
  ...
  template: `
    ...
    >
      <md-fab
        slot="fab"
        icon="add"
        [attr.label]="t('wealth.action.newProposal')"
        (mdClick)="newProposal()"
      ></md-fab>
...
export class RailComponent extends ShowcaseComponent {
  ...
  private readonly totals = getBookTotals();
  ...
  protected badgeFor(destination: Destination): string | null {
    return destination.value === 'proposals' && this.totals.openProposalCount > 0
      ? String(this.totals.openProposalCount)
      : null;
  }
