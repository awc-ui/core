/**
 * Shapes and helpers shared by the holdings screen, its filter bar and its two
 * tables.
 *
 * `<script setup>` cannot contain ES module exports, so everything the React
 * source declared and exported beside its components (`HoldingsFilters.tsx`,
 * `HoldingsTables.tsx`) lives here instead. Every CONTRACT is the React
 * source's, verbatim — only the mechanism (Vue refs and computeds in
 * `usePaging`) is local.
 */
...
import type { T } from '~/composables/useShowcase';
...
export type PopupElement = HTMLElement & {
 ...
 * column just cleared, the re-render carries an unchanged `sort-by` binding and
...
export function usePaging<Row>(rows: ComputedRef<Row[]>, initialRowsPerPage: number) {
  const page: Ref<number> = ref(0);
  const rowsPerPage: Ref<number> = ref(initialRowsPerPage);
  ...
  const lastPage = computed(() => Math.max(0, Math.ceil(rows.value.length / rowsPerPage.value) - 1));
  const safePage = computed(() => Math.min(page.value, lastPage.value));
  const offset = computed(() => safePage.value * rowsPerPage.value);
  const pageRows = computed(() => rows.value.slice(offset.value, offset.value + rowsPerPage.value));

  return { page, rowsPerPage, safePage, offset, pageRows };
}
