 ...
 * which one moved. Four refs and four effects for four switches is the version
 * of this that rots.
 */
...
import { ActionButton } from './HouseholdTabs';
...
export function HouseholdSettingsSheet({
  ...
  open: boolean;
  view: HouseholdView;
  onChange(next: HouseholdView): void;
  onClose(): void;
}) {
  const t = useT();
  ...
  useCustomEvent<CustomEvent>(sheetRef, 'mdClose', () => onClose());

  useCustomEvent<CustomEvent<{ selected: boolean }>>(listRef, 'mdChange', (event) => {
    const target = event.target as HTMLElement | null;
    const key = target?.dataset?.key as keyof HouseholdView | undefined;
    if (!key) return;
    onChange({ ...view, [key]: event.detail.selected });
  });
