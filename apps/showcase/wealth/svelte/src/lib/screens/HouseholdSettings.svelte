<!--
  ...
  shows while you read it — a dialog would blank the thing being configured, and
  §5.5 puts supplementary content at the side on desktop. `variant="modal"`
  ...
  toolbar button, so the reader must be able to get back to that button with the
  keyboard. The modal variant does all of that itself — §9.3 rule 4 says do not
  write your own trap, and there is none here.
  ...
  THREE THINGS THE MANUALS SAY THAT THIS FILE DEPENDS ON:
    ...
    - Slotted `[slot="actions"]` elements are NOT wired by the sheet. Only its
      own built-in close button calls `close()`. That built-in button is the one
      used here, and `mdClose` — which fires for the ✕, the scrim and Escape
      alike — is what puts the state back.
  ...
  they were until the screen was looked at: `<awc-showcase-dock>` is a fixed bar
  across the bottom of the viewport with a z-index above the sheet, so the
  ...
  The switch listener is delegated to the list rather than attached per control:
  `mdChange` bubbles and is composed, and the retargeted `event.target` is the
  `md-switch` host itself, so a `data-` attribute says which one moved. Four
  element bindings for four switches is the version of this that rots.
-->
<script lang="ts" context="module">
  /**
   * What the reader has chosen to see. Pure view state — nothing here is a
   * domain value, which is exactly why it lives in the app and not in the kit.
   */
   ...
   * words this file renders, and all four already exist in the shared
   * dictionary because they name things the console shows elsewhere (the
   * benchmark series, the trend column, the cash asset class, the in-band
   * allocation state).
   */
...
<script lang="ts">
  ...
  export let view: HouseholdView;
  export let onChange: (next: HouseholdView) => void;
  ...
  function onSwitchChange(event: Event) {
    ...
    if (!key) return;
    onChange({ ...view, [key]: (event as CustomEvent<{ selected: boolean }>).detail.selected });
  }
</script>
...
>
  <div class="row row--end">
    <ActionButton icon="restart_alt" onActivate={() => onChange(DEFAULT_VIEW)}>
      {$t('wealth.action.clearFilters')}
    </ActionButton>
  </div>

  <!--
    `selection-mode` is deliberately absent. Turning one on would make the list
    a `listbox` of `option`s and give every row its own selected state — a
    second, competing model beside the switch that is the real control.
    `interaction-mode="multi-action"` is the M3 shape for a settings row: the
    row is a label, the trailing control is the action.
  -->
  <md-list
    class="table-host"
    label={$t('wealth.action.filter')}
    interaction-mode="multi-action"
    on:mdChange={onSwitchChange}
  >
    {#each TOGGLES as toggle (toggle.key)}
      <md-list-item headline={$t(toggle.labelKey)} leading-icon={toggle.icon} lines="1">
        <md-switch
          slot="trailing"
          data-key={toggle.key}
