 ...
 *
 * 13 screens × 3 locales = 39 files, plus one client bundle and six
 ...
 * what the other four builds' fan-out step produces too.
 *
...
/**
 * Every screen, as `{ path, render }`. The household list comes from the
 * fixture rather than a hard-coded table, so adding a household to the kit
 * adds a page here without a second edit — the same contract the SPA builds'
 * fan-out scripts follow.
 */
function routes() {
  return [
    { path: route.overview(), render: (t, locale) => overviewScreen(t, locale) },
    { path: route.holdings(), render: (t, locale) => holdingsScreen(t, locale) },
    ...getHouseholds().map((household) => ({
      path: route.household(household.id),
      render: (t, locale) => householdScreen(t, locale, household.id),
    })),
    { path: route.proposals(), render: (t, locale) => proposalsScreen(t, locale) },
    { path: route.trade(), render: (t, locale) => tradeScreen(t, locale) },
    { path: route.planning(), render: (t, locale) => planningScreen(t, locale) },
  ];
}
...
async function main() {
  ...
  // the kit because all five framework builds share the same grid; the three
  // screen sheets are the ones the React build's screens import (framework-free
  // by design, copied into src/styles/ from the React app).
  ...
  cpSync(
    require_.resolve('@awc-ui/showcase-kit/wealth/app.css'),
    join(dist, 'styles/app.css'),
  );
  for (const sheet of ['trade.css', 'planning.css', 'snackbar.css']) {
    cpSync(join(appRoot, 'src/styles', sheet), join(dist, 'styles', sheet));
  }
