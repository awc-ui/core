 ...
 * conditional on JavaScript in all five builds. Doing the same for the tables
 * would have cost something real, which is why those are written out complete.
 *
 * ONE WEALTH-SPECIFIC TRAP, inherited from the React build: `md-pie-chart`
 * takes `data`, NOT `series`. `pieChart` below writes the right attribute so a
 * screen cannot get it wrong.
 */
...
/**
 * The donut / pie. `data` is the payload prop on this component — the one
 * chart in the family that does not call it `series`. `children` is for the
 * center slot the overview's donut fills.
 */
export function pieChart({ data, config = {}, attributes = {}, children }) {
  return html`<md-pie-chart${attrs({
    data: prop(data),
    'data-chart': JSON.stringify(config),
    ...attributes,
  })}>${children}</md-pie-chart>`;
}
 ...
 * is a closure too.
 *
 * `currency` and `digits` are the same `data-chart` channel, not a second one:
 * a KPI tile plots a EUR total and wants the compact default, while a price
 * series is quoted in the instrument's OWN currency at two digits, and a
 * tooltip that read €412 over a series of dollars would be a wrong number
 * rather than a differently-formatted one. Both are optional and omitted from
 * the payload when absent, so every existing call site is unchanged.
 */
export function sparkline({ data, labels, format = 'currency', currency, digits, attributes = {} }) {
  return html`<md-sparkline${attrs({
    data: prop(data),
    labels: prop(labels),
    'data-chart': JSON.stringify({ format, currency, digits }),
    ...attributes,
  })}></md-sparkline>`;
}

/**
 * The household tree. `nodes` is the structured payload, serialised like every
 * other chart's; `children` carries the `slot="empty"` message, which React
 * renders as a child of the element rather than as a prop.
 */
export function organizationChart({ nodes, attributes = {}, children = '' }) {
  return html`<md-organization-chart${attrs({
    nodes: prop(nodes),
    ...attributes,
  })}>${children}</md-organization-chart>`;
}
