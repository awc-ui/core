...
import { REPORTING_DATE, createRoutes } from '@awc-ui/showcase-kit/design';
...
/*
 * The script is emitted as a file; retain the source integrity check so a `</script>` anywhere inside it
...
if (/<\/script/i.test(PREBOOT_SCRIPT)) {
  console.error('[inject-head] PREBOOT_SCRIPT contains a closing script tag — unexpected closing script tag');
  process.exit(1);
}

const MARKER = '<!-- __AWC_HEAD__ -->';
const head =
  `<script data-awc-preboot src="${createRoutes('angular').basePath}/preboot.js"></script>` +
  `<meta name="awc-reporting-date" content="${REPORTING_DATE}">`;

// External preboot preserves early theme/locale setup under script-src 'self'.
writeFileSync(join(appRoot, 'dist/browser/preboot.js'), PREBOOT_SCRIPT);
