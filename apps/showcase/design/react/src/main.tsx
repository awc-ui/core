...
import { DocumentProvider } from '@/lib/document';
...
/*
 * `ShellProvider`, `DocumentProvider` AND `AppFrame` are all ABOVE the router,
 * for the same reason: `App` returns a different component per route, so
 * anything it renders is unmounted on every navigation.
 *
 * THE DOCUMENT PROVIDER IS THE ONE THIS VERTICAL EXISTS TO DEMONSTRATE. It
 * holds the open document, the selection and the undo history, and an editor
 * that forgot your selection — or worse, your history — because you glanced at
 * the Assets screen would not be an editor.
 */
