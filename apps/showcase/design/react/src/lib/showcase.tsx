import { pictorText } from '@awc-ui/pictor-model';
/**
...
import { DEFAULT_STATE, getShowcaseState, subscribeShowcaseState, type ShowcaseState } from '@awc-ui/showcase-kit/dock';
 ...
 * `t.t('design.label.layers')` at every call site. Screens call `t()` hundreds of
 * times, so the object is flattened into a callable that still carries the
 * locale-bound `format*` methods: `t('design.label.layers')` and
...
function callable(translator: Translator): T {
  const fn = ((key: string, params?: TranslateParams) => translator.has(key) ? translator.t(key, params) : pictorText(translator.locale, key, params)) as T;
...
/** For native event callbacks outside a component; useT binds rendering to locale. */
export function translateUi(message: string, params?: TranslateParams): string {
  return pictorText(getShowcaseState().locale, message, params);
}
