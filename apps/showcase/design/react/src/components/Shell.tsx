...
import { DESTINATIONS, getViewer, type CrumbSpec } from '@awc-ui/showcase-kit/design';
...
import { ComponentLens } from '@/components/ComponentLens';
...
function AppBar() {
  ...
  return (
    <md-app-bar
      ...
      leading-icon-label={compactNav ? undefined : t('design.nav.menu')}
    >
      <span slot="headline" className="shell__brand">{t('design.app.brand')}</span>
      ...
      <div slot="trailing" className="pictor-studio-actions">
        <span className="pictor-save-state" data-state={doc.saveStatus} title={doc.lastSavedAt ? new Date(doc.lastSavedAt).toLocaleString() : p("Autosaves in this browser")}>
          <span className="pictor-save-dot" aria-hidden="true" />
          {doc.saveStatus === 'saved' ? p("Saved locally") : doc.saveStatus === 'unavailable' ? p("Storage unavailable") : p("Saving…")}
        </span>
        <button className="pictor-command-trigger" type="button" onClick={() => window.dispatchEvent(new CustomEvent('pictor:commands'))} aria-label={p("Open command palette")}>
          <span>{p("Quick actions")}</span><kbd>⌘ K</kbd>
        </button>
        <md-button variant="text" icon="widgets" onClick={() => window.dispatchEvent(new CustomEvent('pictor:components'))}>{p("Components")}</md-button>
      </div>
    ...
    <>
      ...
      {actions ? (
        <div className="screen-toolbar">
          ...
          <md-toolbar variant="floating" color="vibrant" aria-label={t('design.nav.toolbar')}>
            {actions}
          </md-toolbar>
        </div>
      ) : null}
