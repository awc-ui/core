...
import { ChoiceTabs, TextControl } from '@/components/editor/controls';
import { useDocument } from '@/lib/document';
import { documentSvg } from '@/lib/editor-model';
...
export function ProfileScreen() {
  ...
  const [view, setView] = useState('recent'), [query, setQuery] = useState('');
  const currentFile = fileById(doc.fileId);
  const files = useMemo(() => (view === 'shared' ? sharedFiles() : recentFiles(12)).filter(file => file.name.toLowerCase().includes(query.toLowerCase())).slice(0, 8), [view, query]);
