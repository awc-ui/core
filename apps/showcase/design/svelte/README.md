...
An interactive design workspace built with Svelte and AWC UI web components. Start with an Orbit, Pulse, or Fieldnotes template, then edit the actual layers on a 960 × 640 canvas.
...
pnpm --filter @awc-ui/showcase-design-svelte dev
```

Open the URL printed by Vite at `/showcase/design/svelte/`. The showcase dock retains theme, language, density, and framework controls; the expanded editing workspace is implemented in this Svelte app.

```sh
pnpm --filter @awc-ui/showcase-design-svelte lint
