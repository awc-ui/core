...
/**
 * Drag-to-zoom, slider panning, hover and the keyboard cursor.
 *
 * All of it hangs off `plotRect()`, which reads the ENGINE's scene — and under
 * mock-doc the engine bails on a zero-sized container, so every handler
 * returned at its first guard and 33 functions were unreachable. Giving the
 * chart host and its canvas a real box makes the engine lay out for real
 * (plot ≈ x 16, y 8, 572 × 307), which is what opens this whole surface.
 */
const HOST_W = 600;
const HOST_H = 400;
...
async function create(attrs = 'zoom="both"') {
  ...
  const canvas = page.root!.shadowRoot!.querySelector('.md-bar-chart__canvas') as HTMLElement | null;
...
describe('md-bar-chart — pointer & keyboard', () => {
  ...
  describe('zoom slider', () => {
    const slider = (page: SpecPage) =>
      page.root!.shadowRoot!.querySelector('.md-bar-chart__zoom-slider') as HTMLElement | null;
  ...
  describe('keyboard cursor', () => {
    const plot = (page: SpecPage) =>
      page.root!.shadowRoot!.querySelector('.md-bar-chart__canvas') as HTMLElement;
    ...
    it('narrates the datum it lands on', async () => {
      ...
      // NOTE md-bar-chart's keyboard cursor only updates the live region — it
      // does NOT emit mdHover the way md-line-chart's does, so the announcement
      // is the observable. It reads "<index>: <series> <value>".
      expect(announcement(page)).toBe('0: S 1');
      restore();
    });
    ...
    it('steps along the categories', async () => {
      ...
      expect(announcement(page)).toBe('0: S 1');
      await press(page, 'ArrowRight');
      expect(announcement(page)).toBe('1: S 2');
      restore();
    });
    ...
    it('clamps at the last category', async () => {
      ...
      expect(announcement(page)).toBe('5: S 6');
      restore();
    });
