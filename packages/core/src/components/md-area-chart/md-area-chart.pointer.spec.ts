...
/**
 * Drag-to-zoom, slider panning and the keyboard cursor.
 *
 * All of it hangs off the ENGINE's scene, and under mock-doc the engine bails
 * on a zero-sized container — so every pointer handler returned at its first
 * guard. This component gates its KEYBOARD handler on the scene too (unlike
 * md-line-chart, which derives its bound from the data), so that path was
 * unreachable as well; md-area-chart.keyboard.spec.ts says so and covers only
 * what did not need layout. Giving the host and its canvas a real box makes the
 * engine lay out for real, which opens both surfaces here.
 */
const HOST_W = 600;
const HOST_H = 400;
...
async function create(attrs = 'zoom="both"') {
  ...
  const canvas = page.root!.shadowRoot!.querySelector('.md-area-chart__canvas') as HTMLElement | null;
...
describe('md-area-chart — pointer & keyboard', () => {
  ...
  describe('zoom slider', () => {
    const slider = (page: SpecPage) =>
      page.root!.shadowRoot!.querySelector('.md-area-chart__zoom-slider') as HTMLElement | null;
  ...
  describe('keyboard cursor', () => {
    const plot = (page: SpecPage) =>
      page.root!.shadowRoot!.querySelector('.md-area-chart__canvas') as HTMLElement;
    ...
    it('narrates the datum it lands on', async () => {
      ...
      // NOTE md-area-chart's keyboard cursor only updates the live region — it
